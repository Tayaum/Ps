...
DIGITS = '0123456789'
LETTERS = string.ascii_letters + '__' + '::'
...
class Parser:
    ...
    def statement(self):
        ...
        if res.error:
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                "Expected 'return', 'continue', 'break', var 'if', 'for', 'while', 'func', 'space', int, float, identifier, '+', '-', '(', '[' or 'not'"
            ))
    ...
    def expr(self):
        ...
        if res.error:
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                "Expected 'var', 'let', 'if', 'for', 'while', 'func', 'space', int, float, identifier, '+', '-', '(', '[' or 'not'"
            ))
    ...
    def comp_expr(self):
        ...
        if res.error:
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                "Expected int, float, identifier, '+', '-', '(', '[', 'if', 'for', 'while', 'func', 'space' or 'not'"
            ))
    ...
    def atom(self):
        ...
        elif tok.matches(TT_KEYWORD, 'space'):
            space_def = res.register(self.space_def())
            if res.error: return res
            return res.success(space_def)
        return res.failure(InvalidSyntaxError(
            tok.pos_start, tok.pos_end,
            "Expected int, float, identifier, '+', '-', '(', '[', if', 'for', 'while', 'func', 'space'"
        ))
    ...
    def space_def(self):
        ...
        if not self.current_tok.matches(TT_KEYWORD, 'space'):
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                f"Expected 'space'"
            ))
        ...
        if self.current_tok.type == TT_ARROW:
            ...
            return res.success(spaceDefNode(
                var_name_tok,
                arg_name_toks,
                body,
                True
...
class BuiltInFunction(BaseFunction):
    ...
    def execute_sleep(self, exec_ctx):
        if isinstance(exec_ctx.symbol_table.get('value'), Number):
            time.sleep(float(str(exec_ctx.symbol_table.get('value'))))
            return RTResult().success(Number.Null)
        else:
    ...
    def execute_rand(self, exec_ctx):
        return RTResult().success(Number(random.uniform(0.000000000000000, 1.0)))
    execute_rand.arg_names = []
    def execute_int(self, exec_ctx):
        if isinstance(exec_ctx.symbol_table.get('value'), Number):
            return RTResult().success(Number(int(float(str(exec_ctx.symbol_table.get('value'))))))
        else:
            return RTResult().failure(RTError(
                self.pos_start, self.pos_end,
                "Second argument must be number", 
    ...
    def execute_round(self, exec_ctx):
        if isinstance(exec_ctx.symbol_table.get('value'), Number):
            return RTResult().success(Number(round(float(str(exec_ctx.symbol_table.get('value'))))))
        else:
            return RTResult().failure(RTError(
                self.pos_start, self.pos_end,
                "Second argument must be number", 
    ...
    def execute_write(self, exec_ctx):
        string = exec_ctx.symbol_table.get('text')
        x = exec_ctx.symbol_table.get('x')
        y = exec_ctx.symbol_table.get('y')
        if isinstance(x, Number):
            if isinstance(y, Number):
                if not y is None and not x is None:
                    sys.stdout.write("\x1b7\x1b[%d;%df%s\x1b8" % (int(str(x)), int(str(y)), str(string)))
                else:
    ...
    def execute_print_ret(self, exec_ctx):
        return RTResult().success(String(str(exec_ctx.symbol_table.get('y'))))
...
class Interpreter:
    ...
    def visit_spaceDefNode(self, node, context):
        res = RTResult()

        space_name = 'Planet::' + node.var_name_tok.value if node.var_name_tok else None
        body_node = node.body_node
        arg_names = [arg_name.value for arg_name in node.arg_name_toks]
        space_value = Function(space_name, body_node, arg_names, node.should_auto_return).set_context(context).set_pos(node.pos_start, node.pos_end)
        
        if node.var_name_tok:
            context.symbol_table.set(space_name, space_value)

        return res.success(space_value)
...
global_symbol_table.set("term::DOWN", BuiltInFunction.DOWN)
global_symbol_table.set("rand", BuiltInFunction.rand)
global_symbol_table.set("argv", String.argv)
global_symbol_table.set("nl", String('\n'))
global_symbol_table.set("print", BuiltInFunction.print)
global_symbol_table.set("term::write", BuiltInFunction.write)
...
def run(fn, text):
    ...
    # Run program
    interpreter = Interpreter()
    context = Context('[blue]<program>[/]')
    context.symbol_table = global_symbol_table
    result = interpreter.visit(ast.node, context)
    if not str(result.value) == '0':
